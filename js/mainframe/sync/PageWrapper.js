...
export default class PageWrapper {
  ...
  async refreshSourcePage() {
    const page = await loadPageForSync(this.page.id);
    const o = $.observable(this);
  ...
  _updateWithSyncStatus(syncStatus) {
    ...
    switch (syncStatus.status) {
      case SyncStatusEnum.TARGET_MISSING:
        property = this.pagesToPush;
        break;
      case SyncStatusEnum.TARGET_UPDATED:
        property = this.pagesToPull;
        break;
      case SyncStatusEnum.CONFLICTING:
        property = this.conflictingPages;
        break;
      ...
      case SyncStatusEnum.TARGET_ATTACHMENT_MISSING:
        property = this.attachmentsToPush;
        break;
      case SyncStatusEnum.TARGET_ATTACHMENT_UPDATED:
        property = this.attachmentsToPull;
        break;
      case SyncStatusEnum.ATTACHMENT_CONFLICTING:
        property = this.conflictingAttachments;
        break;
      // case SyncStatusEnum.ATTACHMENT_UP_TO_DATE: property = this.syncedPages; break; // not used
      case SyncStatusEnum.SOURCE_ATTACHMENT_UPDATED:
        property = this.attachmentsToPush;
        break;
    }
    if (property) {
  ...
  removeFromArray(arr, pageId) {
    const idx = arr.findIndex(s => s.id == pageId);
    if (idx >= 0) {
  ...
  setProgress(action, percent) {
    ...
    $.observable(this.progress).setProperty(
      action,
  ...
  getSourceDiff() {
    return getDiffUrl(
      this.syncStatus.sourcePage,
      this.syncStatus.syncTimeStamp.getPage(this.syncStatus.sourcePage.id).version,
    );
  }

  getTargetDiff() {
    return getDiffUrl(
      this.syncStatus.targetPage,
      this.syncStatus.syncTimeStamp.getPage(this.syncStatus.targetPage.id).version,
    );
  }
  ...
  async computeAllAttachmentsSyncStatus(attachmentListResponse) {
    ...
    if (attachmentListResponse._links.next) {
      ...
      const next = await $.get(`${attachmentListResponse._links.next}&expand=space,version`);
  ...
  async computeAttachmentSyncStatus(sourceAttachment) {
    ...
    let containerId;
    let syncTimeStamp = await SyncTimeStamp.loadLastSyncFromContentWithSpace(
      sourceAttachment.id(),
      this.syncStatus.targetSpaceKey,
    );
    if (this.syncStatus.targetPage && this.syncStatus.targetPage.id) {
      ...
      if (targetAttachment.exists() && syncTimeStamp.isNew()) {
        ...
        syncTimeStamp = await SyncTimeStamp.loadLastSyncFromContentWithSpace(
          targetAttachment.id(),
          sourceAttachment.spaceKey(),
        );
      }
    }
...
function getDiffUrl(page, lastSyncedVersion) {
  return `/pages/diffpagesbyversion.action?pageId=${page.id}&selectedPageVersions=${
    page.version.number
  }&selectedPageVersions=${lastSyncedVersion}`;
}
