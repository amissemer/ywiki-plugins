...
function SyncStatus(pageWrapper, targetSpaceKey, targetPage, syncTimeStamp) {
  const sourcePage = pageWrapper.page;
  ...
  const syncSourceVersion = version(syncTimeStamp.getPage(this.sourcePage.id));
  const syncTargetVersion = version(syncTimeStamp.getOtherPage(this.sourcePage.id));
  if (!targetPage) {
    ...
    syncTimeStamp
    && targetPage.version.number !== syncTargetVersion
    && sourcePage.version.number === syncSourceVersion
  ) {
  ...
  async function createPage() {
    ...
    if (
      pageWrapper.parentPage.syncStatus.status === SyncStatusEnum.TARGET_MISSING
      && !pageWrapper.parentPage.syncStatus.targetPage
    ) {
    ...
    const newPage = Page.copyFrom(sourcePage); // .setVersion(1, `pushed by ysync from ${identifier(sourcePage)}`); (doesn't work on page creation)
    ...
    await ensureEditRestrictions(
      this.targetPage.id,
      this.pageWrapper.pageGroupRoot.editGroup,
  ...
  async function performUpdate() {
    const updatedTargetPage = Page.newVersionOf(
      this.targetPage,
      `pushed by ysync from ${identifier(this.sourcePage)}`,
      true,
    );
    ...
    await ensureEditRestrictions(
      this.targetPage.id,
      this.pageWrapper.pageGroupRoot.editGroup,
      this.sourcePage.body.storage.value,
    );
    await this.syncLabels(false);
    await this.markSynced();
  }
  ...
  async function performPull() {
    ...
    const updatedSourcePage = Page.newVersionOf(this.sourcePage, `pulled by ysync from ${identifier(this.targetPage)}`);
...
SyncStatus.prototype.syncLabels = async function(saveTimeStamp) {
  const sourceLabels = await Labels.getFromPage(this.sourcePage);
  const targetLabels = await Labels.getFromPage(this.targetPage);
  const lastSyncedLabels = this.syncTimeStamp.lastSyncedLabels();
  if (lastSyncedLabels) {
    // can do 3-way merge
  ...
  if (saveTimeStamp) {
    // we may not want to save immediately if we have more changes to save
    await this.syncTimeStamp.save();
  }
};
