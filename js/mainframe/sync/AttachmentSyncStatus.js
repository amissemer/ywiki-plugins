...
function AttachmentSyncStatus(parentSyncStatus, sourceAttachment, targetAttachment, syncTimeStamp) {
  ...
  const syncSourceVersion = version(syncTimeStamp.getPage(this.sourceAttachment.id()));
  const syncTargetVersion = version(syncTimeStamp.getOtherPage(this.sourceAttachment.id()));
  if (!this.targetAttachment || !this.targetAttachment.exists()) {
    // the target doesn't exist
    this.status = SyncStatusEnum.TARGET_ATTACHMENT_MISSING;
    this.performPush = createAttachment;
  } else if (syncTimeStamp.isNew()) {
    // the target exists but we did not find the timestamp
    this.status = SyncStatusEnum.ATTACHMENT_CONFLICTING;
    this.performPush = performUpdate;
    this.performPull = performPull;
  } else if (
    this.targetAttachment.version() !== syncTargetVersion &&
    this.sourceAttachment.version() === syncSourceVersion
  ) {
  ...
  async function createAttachment() {
    ...
    const parentContainerId = this.parentSyncStatus.targetPage.id;
  ...
  async function performUpdate() {
    ...
    log(
      `Pushed attachment ${identifier(this.targetAttachment)} (status=${this.status}) from ${identifier(
  ...
  async function performPull() {
    ...
    log(
      `Pulled attachment ${identifier(this.sourceAttachment)} (status=${this.status}) from ${identifier(
