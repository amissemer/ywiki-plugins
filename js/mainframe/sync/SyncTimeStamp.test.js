...
const page1Id = '257597020'; // in ~adrien.missemer@sap.com
const page2Id = '404455459'; // in ps

describe('SyncTimeStamp', () => {
  it('should handle load and save', async () => {
    // setup (clean)
    const page1 = await getContentById(page1Id, 'version,space');
    const page1space = page1.space.key;
    const page2 = await getContentById(page2Id, 'version,space');
    const page2space = page2.space.key;
  ...
  it('lastSyncedLabels should not overwrite the syncedPages', async () => {
    // setup (clean)
    const page1 = await getContentById(page1Id, 'version,space');
    const page1space = page1.space.key;
    const page2 = await getContentById(page2Id, 'version,space');
  ...
  it('setSyncedPages should not overwrite the syncedLabels', async () => {
    // setup (clean)
    const page1 = await getContentById(page1Id, 'version,space');
    const page1space = page1.space.key;
    const page2 = await getContentById(page2Id, 'version,space');
