...
$(() => {
  ...
  const appElt = $('ci-sync-app').first();
  const globalOptions = {
  ...
  const conflictViewElt = $('ci-sync-conflicts').first();
  ...
  loadTemplate('sync-plugin/page-groups-table.html').then(tmpl => {
  ...
  if (conflictViewElt.length) {
    loadTemplate('sync-plugin/sync-conflicts-table.html').then(tmpl => {
