 ...
 */
const windowEventListener = (function windowEventListener() {
  // Start listening to messages (from other frames, typically)
  const eventMethod = window.addEventListener ? 'addEventListener' : 'attachEvent';
  const eventer = window[eventMethod];
  const messageEvent = eventMethod == 'attachEvent' ? 'onmessage' : 'message';
  const requestListeners = {}; // map of key=action, value=function(correlationId, payload)
  const responseListeners = {}; // map of key=correlationId, value={ successHandler: function(responsePayload), errorHandler: function(errorPayload)  }
  ...
  function eventCallback(e) {
    if (e.data.action) {
      ...
      if (responseListeners[e.data.correlationId]) {
        ...
        const responseListener = responseListeners[e.data.correlationId];
  ...
  function registerResponseListener(correlationId, listener) {
    if (!listener.successHandler && !listener.errorHandler) {
      console.error(
        'Cannot register response listener as it is missing a successHandler function or errorHandler function',
        listener,
      );
      return;
    }
