...
describe('Attachment', () => {
  it('should load, clone and delete', async () => {
  ...
  it('should clone from another attachment', async () => {
    const from = await Attachment.getOrCreateAttachment(pageToCopyAttachmentsTo, attachmentTitle);
    const to = await Attachment.getOrCreateAttachment(pageToCopyAttachmentsTo, `${attachmentTitle}_2`);
  ...
  it('existing attachment should return version() and downloadUrl()', async () => {
    const att = await Attachment.getOrCreateAttachment(pageToCopyAttachmentsTo, attachmentTitle);
    assert.isNotNull(att.id());
  ...
  it('unexisting attachment should return null version() and downloadUrl()', async () => {
    const att = await Attachment.getOrCreateAttachment(pageToCopyAttachmentsTo, 'notAttitle');
    assert.isNull(att.id());
