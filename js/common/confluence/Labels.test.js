...
describe('Labels', () => {
  it('should load (lazily or not) and save properly', async () => {
    // set to
  ...
  it('should remove or add labels', async () => {
    // set to
    let labels1 = await loadLabels(page1Id, true);
    let labels2 = await loadLabels(page2Id);
  ...
  it('should merge labels', async () => {
    ...
    labels1 = await loadLabels(page1Id);
    labels2 = await loadLabels(page2Id, true);
    const initial = ['a', 'b', 'c']; // page2 added z and removed a (and page 1 added 'd')
...
async function loadLabels(pageId, lazyLoad) {
  const expand = lazyLoad ? '' : 'metadata.labels';
  const page = await getContentById(pageId, expand);
