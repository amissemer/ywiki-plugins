...
Page.copyFrom = function(page) {
  ...
  const parentId = page.ancestors && page.ancestors.length ? page.ancestors[0].id : 0;
  const body = page.body && page.body.storage ? page.body.storage.value : null;
  return new Page(page.title, body, spaceKey, parentId);
};
Page.newVersionOf = function(page, message, isMajorEdit) {
  return Page.copyFrom(page)
    ._setId(page.id)
    .setVersion(page.version.number + 1, message, isMajorEdit);
};
export default Page;
